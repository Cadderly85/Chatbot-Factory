#!/usr/bin/env python3
"""Chatbot Factory - Agent Combine Texte + Vocal pour HF Space"""
...
try:
    ...
    if kb.exists():
        ...
        logger.info(f"KB: {len(knowledge_docs)} docs")
except:
    pass

SYSTEM_PROMPT = "Tu es l'assistant de Clinique Dentaire Sourire a Montreal. Services: Nettoyage 150-250$, Blanchiment 400-600$, Orthodontie, Implants. Tel: (514) 555-0123. Chaleureux, concis (1-3 phrases), francais. Jamais de diagnostic medical."
...
@app.get("/")
async def root():
    return PlainTextResponse("Chatbot Factory OK")
...
try:
    ...
    logger.info("Vocal indisponible")

@app.get("/vocal/health")
async def vh():
    return {"status": "ok", "vocal_available": vocal_ok, "whisper_loaded": whisper_model is not None}

@app.post("/vocal/transcribe-and-respond")
async def vocal(audio: UploadFile = File(...), language: str = Form("fr"), session_id: str = Form("default")):
    if not vocal_ok:
